...
/**
 * v94: Adapter - convert segments for useAudioTranslation.addSegments.
 * Uses TEXT-LENGTH-BASED TIMING: each segment's duration is proportional
 * to its word count, then all durations are scaled to fit the total video duration.
 * This ensures short phrases (like "Hey guys!") get short durations (~3s)
 * and long monologues get long durations (~20s), with no gaps or overlaps.
 * 
 * v94: Now includes targetLanguage in each segment so the TTS hook/route
 * can enforce the correct output language regardless of source text language.
 * 
 * v104: Calculates targetDuration with scaledDuration fallback.
 * v103 bug: raw segments have NaN timing, so targetDuration was always undefined.
 * v104 fix: falls back to scaledDuration when original timing unavailable.
 * This ensures v77 TTS route ALWAYS receives targetDuration for duration matching.
 */
...
function adaptSegmentsForAudio(segments: TranscriptSegment[], targetLanguage: string = 'en'): Array<{
  ...
  // v81: Diagnostic log on first call - shows actual segment shape
  if (!_hasLoggedSegmentShape && segments.length > 0) {
    _hasLoggedSegmentShape = true;
    const sample = segments[0] as any;
    console.log('[v129] 🔍 Segment shape diagnostic (first segment):', {
  ...
  // Estimate total duration from last segment with valid timing
  ...
  console.log('[v129] 📊 Adapting segments with text-length-based timing...');
  
  // === STEP 1: Calculate raw duration for each segment based on text length ===
  // Using 150 words-per-minute as baseline speech rate, minimum 0.5s per segment
  const rawDurations = segments.map(seg => {
    const text = seg.text || '';
    const wordCount = text.split(/\s+/).filter(w => w.length > 0).length;
    const rawDuration = Math.max((wordCount / 150) * 60, 0.5); // 150 WPM, min 0.5s
    return rawDuration;
  });
  
  // === STEP 2: Calculate total raw duration and scaling factor ===
  ...
  console.log('[v129] Total raw duration (unscaled):', totalRawDuration.toFixed(2), 's');
  console.log('[v129] Scale factor:', scaleFactor.toFixed(3), 'to fit', totalDurationEstimate, 's');
  
  // === STEP 3: Create adapted segments with scaled durations and cumulative start times ===
  // v103: Also calculate original transcription duration for duration matching
  ...
  const adapted = segments.map((seg, index) => {
    ...
    // v103: Calculate original transcription duration from raw segment timing
    // This is the actual speaking duration in the source video
    const origStartSec = resolveNumber(
      seg.start,
      raw.startMs != null ? raw.startMs / 1000 : undefined,
      raw.startTime
    );
    const origEndSec = resolveNumber(
      raw.end,
      raw.endMs != null ? raw.endMs / 1000 : undefined,
      raw.endTime
    );
    const origDurationSec = resolveNumber(
      raw.duration,
      raw.durationMs != null ? raw.durationMs / 1000 : undefined
    );
    
    // v104 FIX: Use scaledDuration as final fallback for targetDuration
    // v103 bug: segments have start: NaN, end: NaN → origStartSec/origEndSec are null
    // → targetDuration was ALWAYS undefined → v77 TTS route never got duration matching
    let targetDuration: number;
    let targetDurationSource: string;
    if (origEndSec != null && origStartSec != null && origEndSec > origStartSec) {
      targetDuration = origEndSec - origStartSec;
      targetDurationSource = 'original';
    } else if (origDurationSec != null && origDurationSec > 0) {
      targetDuration = origDurationSec;
      targetDurationSource = 'rawDuration';
    } else {
      // v104 FIX: Use scaledDuration as fallback - this is proportional to word count
      // and ensures v77 TTS route ALWAYS receives a targetDuration for duration matching
      targetDuration = scaledDuration;
      targetDurationSource = 'scaledFallback';
    }
    ...
    return {
      ...
      speakerGender: raw.speakerGender,
      // v94: Force English language for all TTS to prevent language switching
      // when source audio briefly switches to Portuguese or other languages
      targetLanguage: targetLanguage,
      // v104: Pass targetDuration for TTS duration matching (ALWAYS has a value now)
      targetDuration: targetDuration,
      _targetDurationSource: targetDurationSource, // diagnostic only
    };
  });
  
  // v104: Log targetDuration source distribution
  const sourceCount = { original: 0, rawDuration: 0, scaledFallback: 0 };
  adapted.forEach((s: any) => {
    if (s._targetDurationSource) sourceCount[s._targetDurationSource as keyof typeof sourceCount]++;
  });
  console.log(`[v129] 🎯 targetDuration sources: original=${sourceCount.original}, rawDuration=${sourceCount.rawDuration}, scaledFallback=${sourceCount.scaledFallback}`);
  if (sourceCount.scaledFallback > 0) {
    console.log(`[v129] ℹ️ ${sourceCount.scaledFallback} segments using scaledDuration as targetDuration (original timing was NaN/missing)`);
  }
  
  // v94: Log language enforcement
  console.log(`[v129] 🌐 Enforcing target language '${targetLanguage}' for all ${adapted.length} TTS segments`);
  
  // v81: Log summary of adaptation results
  const validCount = adapted.filter(s => !isNaN(s.start) && s.duration > 0).length;
  if (validCount < adapted.length) {
    console.warn(`[v129] ⚠ Segment adaptation: ${validCount}/${adapted.length} segments have valid timing after fix`);
  } else {
    console.log(`[v129] ✅ All ${adapted.length} segments adapted with text-based timing`);
  }
  
  // v93: Detailed segment timing diagnostics
  console.log('[v129] 📊 Segment timing summary:');
  console.log('[v129] Total segments:', adapted.length);
  console.log('[v129] Video duration estimate:', totalDurationEstimate, 'seconds');
  const avgDur = adapted.reduce((sum, s) => sum + s.duration, 0) / Math.max(adapted.length, 1);
  console.log('[v129] Average segment duration:', avgDur.toFixed(2), 'seconds');
  const minDur = Math.min(...adapted.map(s => s.duration));
  const maxDur = Math.max(...adapted.map(s => s.duration));
  console.log('[v129] Duration range:', minDur.toFixed(2), 's -', maxDur.toFixed(2), 's (ratio:', (maxDur / Math.max(minDur, 0.01)).toFixed(1) + 'x)');
  
  // Log first 5 and last 5 segments to see the timing spread
  console.log('[v129] First 5 segments:');
  adapted.slice(0, 5).forEach((seg, i) => {
    const wordCount = seg.text.split(/\s+/).filter((w: string) => w.length > 0).length;
    console.log(`  [${i}] ${seg.start.toFixed(2)}s - ${(seg.start + seg.duration).toFixed(2)}s (${seg.duration.toFixed(2)}s, ${wordCount} words) - "${seg.text.substring(0, 30)}..."`);
  });

  if (adapted.length > 5) {
    console.log('[v129] Last 5 segments:');
    adapted.slice(-5).forEach((seg, i) => {
      const actualIndex = adapted.length - 5 + i;
      const wordCount = seg.text.split(/\s+/).filter((w: string) => w.length > 0).length;
      console.log(`  [${actualIndex}] ${seg.start.toFixed(2)}s - ${(seg.start + seg.duration).toFixed(2)}s (${seg.duration.toFixed(2)}s, ${wordCount} words) - "${seg.text.substring(0, 30)}..."`);
    });
  }

  // Check for gaps and overlaps (should be zero with cumulative approach)
  console.log('[v129] Checking for gaps/overlaps in coverage...');
  let gapCount = 0;
  let overlapCount = 0;
  let totalGapTime = 0;
  for (let i = 1; i < adapted.length; i++) {
    const prevEnd = adapted[i-1].start + adapted[i-1].duration;
    const currStart = adapted[i].start;
    const gap = currStart - prevEnd;
    if (gap > 1.0) { // Gap larger than 1 second
      gapCount++;
      totalGapTime += gap;
      console.warn(`[v129] ⚠ GAP: ${gap.toFixed(2)}s between seg ${i-1} (ends ${prevEnd.toFixed(2)}s) and seg ${i} (starts ${currStart.toFixed(2)}s)`);
    } else if (gap < -0.1) { // Overlap larger than 0.1 second
      overlapCount++;
      console.warn(`[v129] ⚠ OVERLAP: ${(-gap).toFixed(2)}s between seg ${i-1} (ends ${prevEnd.toFixed(2)}s) and seg ${i} (starts ${currStart.toFixed(2)}s)`);
    }
  }
  if (gapCount === 0 && overlapCount === 0) {
    console.log('[v129] ✅ No significant gaps or overlaps detected');
  } else {
    console.log(`[v129] 📊 Found ${gapCount} gaps (total ${totalGapTime.toFixed(2)}s uncovered) and ${overlapCount} overlaps`);
  }
  
  // Log total coverage vs video duration
  if (adapted.length > 0) {
    const firstStart = adapted[0].start;
    const lastEnd = adapted[adapted.length - 1].start + adapted[adapted.length - 1].duration;
    const totalSegmentTime = adapted.reduce((sum, s) => sum + s.duration, 0);
    console.log(`[v129] 📊 Coverage: segments span ${firstStart.toFixed(2)}s to ${lastEnd.toFixed(2)}s (${(lastEnd - firstStart).toFixed(2)}s range)`);
    console.log(`[v129] 📊 Total segment audio time: ${totalSegmentTime.toFixed(2)}s vs video estimate: ${totalDurationEstimate.toFixed(2)}s`);
  }
  
  // v103: Duration matching diagnostics
  const withTargetDuration = adapted.filter(s => s.targetDuration != null && s.targetDuration > 0);
  if (withTargetDuration.length > 0) {
    const avgTargetDur = withTargetDuration.reduce((sum, s) => sum + s.targetDuration!, 0) / withTargetDuration.length;
    const avgScaledDur = withTargetDuration.reduce((sum, s) => sum + s.duration, 0) / withTargetDuration.length;
    const diffs = withTargetDuration.map(s => Math.abs(s.duration - s.targetDuration!));
    const avgDiff = diffs.reduce((sum, d) => sum + d, 0) / diffs.length;
    const maxDiff = Math.max(...diffs);
    
    console.log(`[v129] 🎯 Duration Matching Summary:`);
    console.log(`[v129]   Segments with original timing: ${withTargetDuration.length}/${adapted.length}`);
    console.log(`[v129]   Avg original duration: ${avgTargetDur.toFixed(2)}s`);
    console.log(`[v129]   Avg scaled duration:   ${avgScaledDur.toFixed(2)}s`);
    console.log(`[v129]   Avg difference:        ${avgDiff.toFixed(2)}s`);
    console.log(`[v129]   Max difference:        ${maxDiff.toFixed(2)}s`);
    
    // Log first 3 segments showing targetDuration vs scaled duration
    console.log('[v129] 🎯 First 3 segments with duration matching:');
    withTargetDuration.slice(0, 3).forEach((seg, i) => {
      console.log(`  [${i}] targetDuration=${seg.targetDuration!.toFixed(2)}s, scaledDuration=${seg.duration.toFixed(2)}s, diff=${Math.abs(seg.duration - seg.targetDuration!).toFixed(2)}s - "${seg.text.substring(0, 30)}..."`);
    });
  } else {
    console.log('[v129] ⚠ No segments had original transcription timing - targetDuration not available for duration matching');
  }
...
export default function AudioClarification({
  ...
  useEffect(() => {
    ...
    if (
      ...
      const speakerData = speakers.map(s => {
        ...
        return {
          id: s.id,
          gender: s.gender as 'male' | 'female' | 'neutral' | 'unknown',
      ...
      audioTranslation.initializeVoices(speakerData as any);
  ...
  const bufferStatus = useMemo(() => {
    const loadedChunks = chunks.filter(c => (c.status as string) === 'ready' || c.status === 'loaded').length;
